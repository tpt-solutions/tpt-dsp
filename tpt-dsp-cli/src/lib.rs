...
use tpt_dsp_analysis::{
    find_peaks, linear_to_db, spectral_centroid, zero_crossing_rate, Averaging,
    RealtimeSpectrumAnalyzer, SpectrumConfig, DEFAULT_DB_FLOOR,
};
...
impl Effect {
    ...
    pub fn parse(spec: &str) -> Result<Self> {
        let (kind, rest) = spec
            .split_once(':')
            .ok_or_else(|| anyhow::anyhow!("effect needs a `<kind>:` prefix: `{spec}`"))?;
        ...
        Ok(match kind {
            "biquad" => {
                let type_name = *fields
                    .first()
                    .ok_or_else(|| anyhow::anyhow!("biquad needs a type"))?;
            ...
            "waveshaper" => {
                let curve = match *fields
                    .first()
                    .ok_or_else(|| anyhow::anyhow!("waveshaper needs a curve"))?
                {
                    "tanh" => Curve::Tanh,
        ...
        for effect in effects {
            let boxed: Box<dyn Processor> = match effect {
                Effect::Biquad {
                    ...
                    Box::new(BiquadProc(Biquad::<f32>::design(
                        *kind,
                        sample_rate,
                        *freq,
                        *q,
...
pub fn analyze_complex(
    ...
    let half = n / 2;
    let (dom_bin, dom_val) = avg[1..=half]
        .iter()
        .enumerate()
        .map(|(i, &v)| (i + 1, v))
        .fold(
    ...
    let db_half: Vec<f64> = avg[..=half]
        .iter()
        .map(|&m| linear_to_db(m as f32, 1.0, DEFAULT_DB_FLOOR) as f64)
        .collect();
    let half_mag_f32: Vec<f32> = avg[..=half].iter().map(|&m| m as f32).collect();
    let db_half_f32: Vec<f32> = db_half.iter().map(|&d| d as f32).collect();
    let centroid_bins = spectral_centroid(&half_mag_f32) as f64;
    let mut peaks: Vec<(usize, f64)> = find_peaks(&db_half_f32, 0.1)
        .into_iter()
        .map(|b| (b, db_half[b]))
        .collect();
    peaks.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    peaks.truncate(top);
    let spectrum = (0..=half)
        .map(|b| (b as f64 * bin_width, db_half[b]))
        .collect();
...
pub fn write_spectrum_csv(path: &Path, spectrum: &[(f64, f64)]) -> Result<()> {
    let mut file =
        std::fs::File::create(path).with_context(|| format!("create csv `{}`", path.display()))?;
    use std::io::Write;
...
pub fn demod_file(
    ...
    let out_rate = if let Some(m) = decimate {
        iq_rate / m as f64
    } else {
        iq_rate
    };
...
mod tests {
    ...
    fn effect_spec_parsing() {
        assert!(matches!(
            Effect::parse("biquad:lowpass:1000").unwrap(),
            Effect::Biquad {
        ...
        assert!(matches!(
            Effect::parse("waveshaper:tanh:5:1").unwrap(),
            Effect::Waveshaper {
                curve: Curve::Tanh,
                ..
            }
        ...
        assert!(matches!(
            Effect::parse("waveshaper:poly:1,2,3,4").unwrap(),
            Effect::Waveshaper {
                curve: Curve::Polynomial(_),
                ..
            }
        ));
        match Effect::parse("eq:1000,6,1;2000,-3,1").unwrap() {
            Effect::Eq { bands } => {
                assert_eq!(bands, vec![(1000.0, 6.0, 1.0), (2000.0, -3.0, 1.0)])
            }
    ...
    fn chain_lowpass_attenuates_a_wav() {
        ...
        .unwrap();
        let before = data.channels[0]
            .iter()
            .map(|x| x.abs())
            .fold(0.0f32, f32::max);
        chain.process_channel(&mut data.channels[0]);
        let after = data.channels[0]
            .iter()
            .map(|x| x.abs())
            .fold(0.0f32, f32::max);
        assert!(
            after < before,
            "low-pass should reduce a 9.6 kHz tone: {before} -> {after}"
        );
    }
    ...
    fn analyze_real_finds_sine_peak() {
        ...
        assert!(
            (report.dominant_hz - 1000.0).abs() < 2.0,
            "dom {}",
            report.dominant_hz
        );
