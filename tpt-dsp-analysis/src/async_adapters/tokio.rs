//! tokio channel adapters (feature `async-tokio`).
...
pub async fn process_channel<F>(mut rx: Receiver<Vec<f32>>, tx: Sender<Vec<f32>>, mut f: F)
where
    F: FnMut(&[f32], &mut [f32]),
{
...
pub async fn process_channel_in_place<F>(mut rx: Receiver<Vec<f32>>, tx: Sender<Vec<f32>>, mut f: F)
where
    F: FnMut(&mut [f32]),
{
...
mod tests {
    ...
    fn channel_in_place_pipeline() {
        ...
        rt.block_on(async {
            let (in_tx, in_rx) = tokio::sync::mpsc::channel(4);
            let (out_tx, mut out_rx) = tokio::sync::mpsc::channel(4);
            let handle = tokio::spawn(process_channel_in_place(
                in_rx,
                out_tx,
                |frame: &mut [f32]| {
    ...
    fn stream_pipeline_terminates() {
        ...
        rt.block_on(async {
            ...
            let (out_tx, mut out_rx) = tokio::sync::mpsc::channel(4);
            process_stream(src, out_tx, |in_: &[f32], out: &mut [f32]| {
